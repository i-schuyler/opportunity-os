...
export function createBillingApiRequestHandler({
  ...
  const persistentStoreRequired =
    typeof requirePersistentStore === 'boolean' ? requirePersistentStore : isRealOperationMode(operationMode);
...
export function createEnvPersistentBillingStore(globalObject = globalThis) {
  ...
  const readState = () => {
    ...
    const raw = fsAdapter.readFileSync(filePath, 'utf8');
    return JSON.parse(String(raw || '{}'));
  };

  const writeState = (state) => {
    fsAdapter.writeFileSync(filePath, JSON.stringify(state));
  };
