...
function isLocalTestHarness(runtimeEnv) {
  return Boolean(runtimeEnv && runtimeEnv.__opportunityLocalTestHarness === true);
}
...
export function createBillingApiRequestHandler({
  ...
  const persistentStoreRequired = typeof requirePersistentStore === 'boolean'
    ? requirePersistentStore
    : resolvedOperationMode
      ? isRealOperationMode(resolvedOperationMode)
      : !isLocalTestHarness(runtimeEnv);
