...
export function createPersistentBillingStore({ readState, writeState } = {}) {
  ...
  let state = cloneDefaultStoreData();
  let initializationError = '';

  try {
    state = normalizeStoreData({ ...cloneDefaultStoreData(), ...(readState() || {}) });
  } catch (error) {
    initializationError =
      error && error.message ? `Persistent billing store read failed: ${error.message}` : 'Persistent billing store read failed.';
  }
  ...
  return {
    isPersistent: true,
    isHealthy: initializationError.length < 1,
    initializationError,
    getEntitlement(userId) {
      ensureHealthy();
      return state.entitlements[String(userId || '')] || null;
